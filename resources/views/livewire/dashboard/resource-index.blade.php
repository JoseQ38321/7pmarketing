<div class="bg-white p-5 rounded-lg">
    <div class="flex items-center justify-between">
        <div class="flex gap-2.5">
            <input class="rounded-lg border-gray-200 shadow-sm focus:ring-0" type="text" wire:model="search" placeholder="{{ __('Buscar un Resource') }}">
        ...
        <div class="flex gap-2.5">
            ...
            <a href="{{ route('resource.create') }}" class="flex items-center gap-1 py-1.5 px-4 rounded-lg border border-gray-200 shadow-sm bg-white text-gray-700">
                    ...
                    <tr class="border-b border-gray-100 hover:bg-gray-50">
                        <td class="py-3 px-6 text-left">
                            <div class="flex items-center gap-4">
                                <img class="h-14 w-16 rounded-sm object-cover" src="{{ $resource->image }}" alt="{{ $resource->title }}">
                                <div>
                                    <div>
                                        <a href="#" class="font-bold text-gray-800">{{ $resource->title }}</a>
                        ...
                        <td class="py-3 px-6 text-left w-64">
                            <div>
                                <p class="mb-2">{{ __('Publicado por') }}: <br> {{ $resource->user->name }} {{ $resource->created_at->diffForHumans() }}</p>
                        ...
                        <td class="py-3 px-6 text-center">
                            <div class="flex item-center justify-center">
                                ...
                                <a href="{{ route('resource.edit', $resource) }}" class="w-4 mr-2 transform hover:text-blue-500 hover:scale-110 transition-transform ease-in-out cursor-pointer">
                                ...
                                <div class="w-4 mr-2 transform hover:text-red-500 hover:scale-110 transition-transform ease-in-out cursor-pointer" wire:click="confirmResourceDeletion( {{ $resource->id }} )" wire:loading.attr="disabled">
    ...
    <x-jet-dialog-modal wire:model="confirmingResourceDeletion">
        <x-slot name="title">
            Eliminar recurso
        </x-slot>

        <x-slot name="content">
            Estás seguro de que deseas eliminar este recurso, recuerda que esta accion no se puede revertir.
        </x-slot>

        <x-slot name="footer">
            <x-jet-secondary-button wire:click="$set('confirmingResourceDeletion', false)" wire:loading.attr="disabled">
                Cancelar
            </x-jet-secondary-button>

            <x-jet-danger-button class="ml-2" wire:click="deleteResource({{ $confirmingResourceDeletion }})" wire:loading.attr="disabled">
                Eliminar Recurso
            </x-jet-danger-button>
        </x-slot>
    </x-jet-dialog-modal>
